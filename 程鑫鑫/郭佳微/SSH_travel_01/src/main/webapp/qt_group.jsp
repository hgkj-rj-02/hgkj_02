...
<div class="tuangouy">
    <ul>
        <c:forEach items="${lineList}" var="linel">
            ...
            <li>
                <div><a href="allLineDetail.action?line.lineId=${linel.lineId}">
                    <c:forEach items="${linel.pictureSet}" var="line2" begin="0" end="0">
                    <img src="${line2.pictName}" width="305" height="200" />
                    </c:forEach>
                </a></div>
                <div class="tuangouy-m"><a href="allLineDetail.action?line.lineId=${linel.lineId}">${linel.lineName}</a><br />
                    简介：${linel.lineIntroduction}<br/>
                    <span>￥${linel.lineTeamBuyPrice}</span>&nbsp;&nbsp;原价：￥${linel.linePrice}
                </div>
